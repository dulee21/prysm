...
// 핫상태는 자주 변경되거나 접근되는 데이터를 의미한다. 즉 최신 블록과 관련된 데이터를 의미한다.
// 이 데이터는 더 빠르게 접근될 수 있어야 해 메모리 같은 곳에 저장된다.
// 반면 최종화된 데이터는 변경되지 않는 것이니 파일 시스템 같은 곳에 저장된다.

// This defines how many epochs since finality the run time will begin to save hot state on to the DB.
// 이 값은 최종화된	이후 몇 개의 에포크가 지나면 런타임이 핫 상태를 DB에 저장하기 시작할지를 정의합니다.
// 즉 100개의 에포크가 지나야 핫상태 데이터를 저장하여 데이터를 영속성으로 저장하고 메모리 사용을 최적화한다.
var epochsSinceFinalitySaveHotStateDB = primitives.Epoch(100)

// 아마 최종화가 되면 영속성으로 저장하기 때문에 캐시가 줄어드는 것 같다.
// 에포크가 지나서 캐시 크기를 확장하고 최종화가 되면 주는 방식으로 구현된듯 하다.

// This defines how many epochs since finality the run time will begin to expand our respective cache sizes.
// 이 값은 최종화된 이후 몇 개의 에포크가 지나면 런타임이 각각의 캐시 크기를 확장하기 시작할지를 정의합니다.
...
//
// ReceiveBlock 함수는 수신된 블록에 대해 수행되는 작업(pubsub 제외)을 정의합니다. 작업은 다음으로 구성됩니다:
//  1. 블록을 검증하고, 상태 전이를 적용하고 체크포인트를 업데이트합니다.
//  2. 처리된 블록에 포크 선택을 적용합니다.
//  3. 최신 헤드 정보를 저장합니다.
func (s *Service) ReceiveBlock(ctx context.Context, block interfaces.ReadOnlySignedBeaconBlock, blockRoot [32]byte, avs das.AvailabilityStore) error {
	fmt.Println("ReceiveBlock 함수 호출")
	...
	// 현재 증명된 에포크와 최종화된 에포크를 저장하여 나중에 사용합니다.
...
func (s *Service) ReceiveBlockBatch(ctx context.Context, blocks []blocks.ROBlock, avs das.AvailabilityStore) error {
	fmt.Println("ReceiveBlockBatch 함수 호출")
