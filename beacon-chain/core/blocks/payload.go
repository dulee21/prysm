...
func ValidatePayloadWhenMergeCompletes(st state.BeaconState, payload interfaces.ExecutionData) error {
	...
	// fmt.Println("--- 1. 현재 payload의 부모 블록이 state의 마지막 paylaod 헤더의 블록 hash가 같은지 확인 ---")
	header, err := st.LatestExecutionPayloadHeader()
	if err != nil {
		return err
	}
	...
	// fmt.Println("state payload 헤더의 블록 hash : ", header.BlockHash())
	// fmt.Println("현재 payload의 부모 블록의 hash : ", payload.ParentHash())
	return nil
}
...
func ValidatePayload(st state.BeaconState, payload interfaces.ExecutionData) error {
	...
	// fmt.Println("--- 2. payload의 prev randao가 state의 현재 epoch의 randao mix와 같은지 확인 ---")
	// fmt.Println("state의 현재 epoch의 randao mix : ", random)
	// fmt.Println("payload의 prev randao : ", payload.PrevRandao())
	if !bytes.Equal(payload.PrevRandao(), random) {
		return ErrInvalidPayloadPrevRandao
	}

	// fmt.Println("--- 3. payload의 timestamp가 state의 현재 slot의 timestamp와 같은지 확인 ---")
	t, err := slots.ToTime(st.GenesisTime(), st.Slot())
	if err != nil {
		return err
	}
	// fmt.Println("state의 현재 slot의 timestamp : ", t.Unix())
	// fmt.Println("payload의 timestamp : ", payload.Timestamp())
...
func ProcessPayloadHeader(st state.BeaconState, header interfaces.ExecutionData) (state.BeaconState, error) {
	// fmt.Println("PayloadHeader 처리 시작")
	var err error
	if st.Version() >= version.Capella {
