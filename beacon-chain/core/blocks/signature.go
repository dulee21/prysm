...
func VerifyBlockSignature(beaconState state.ReadOnlyBeaconState,
	...
	// 주어진 슬롯 번호를 기반으로 해당 슬롯이 속한 Epoch 번호를 계산하여 반환한다.
	currentEpoch := slots.ToEpoch(beaconState.Slot())
	// fmt.Println("현재 에포크 번호 : ", currentEpoch)

	// 도메인의 결과값은 포크 버전 (최근 적용된 포크 버전)과 제네시스 검증자 루트를 통해 계산된다.
	// 32바이트 크기의 해시값으로 반환된다.
	// 도메인 타입은 4바이트이다. 현재는 블록 제안자를 검증하기 위해 사용되기 때문에 비콘 제안자 도메인 타입을 사용한다.
	// 도메인 타입은 블록 서명, 랜다오 서명, 등 다양한 서명 유형을 구분하는데 사용된다.
