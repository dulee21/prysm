...
func ProcessBLSToExecutionChanges(
	...
	fmt.Println("BLSToExecutionChanges 처리 시작")
...
func ProcessWithdrawals(st state.BeaconState, executionData interfaces.ExecutionData) (state.BeaconState, error) {
	fmt.Println("----- 출금 처리 시작 -----")

	// expected_withdrawals = get_expected_withdrawals(state)
	// 예상 출금 목록을 가져온다
	// 예상 출금 목록 조건
	// 검증자 중 32 ETH 초과로 staking 중이어야한다.
	// 출금 대상 검증자는 출금 대상 검증자 목록에 포함되어야한다.
	expectedWithdrawals, err := st.ExpectedWithdrawals()
	if err != nil {
		return nil, errors.Wrap(err, "could not get expected withdrawals")
	}
	fmt.Println("--- 1. 예상 출금 목록을 가져온다.")
	// fmt.Println("expectedWithdrawals:", expectedWithdrawals)
	// fmt.Printf("expectedWithdrawals: %x\n", expectedWithdrawals)
	if len(expectedWithdrawals) > 0 {
		withdrawal := expectedWithdrawals[0]
		fmt.Printf("Address: %x, Amount: %d\n", withdrawal.Address, withdrawal.Amount)
	}

	// 실행 페이로드의 출금 목록을 가져온다.
	// 실행 데이터가 블라인드 상태인지 확인한다.
	var wdRoot [32]byte
	...
	// fmt.Println("--- 2. 실행 페이로드의 출금 Hash를 가져온다.")
	// fmt.Println("wdRoot:", wdRoot)
	...
	// fmt.Println("--- 3. 예상 출금 목록과 실행 페이로드의 출금 목록의 루트를 비교한다.")
	// fmt.Println("expectedRoot : ", expectedRoot)
	// fmt.Println("wdRoot : ", wdRoot)

	// fmt.Println("--- 4. 검증자 출금 처리를 시작한다.")
	for _, withdrawal := range expectedWithdrawals {
		// decrease_balance(state, withdrawal.validator_index, withdrawal.amount)
		// fmt.Println("withdrawal.ValidatorIndex:", withdrawal.ValidatorIndex)
		// fmt.Println("withdrawal.Amount:", withdrawal.Amount)
	...
	// fmt.Println("--- 5. 다음 출금 인덱스를 업데이트한다.")
	// 내가추가한 코드
	// nextIndex, err := st.NextWithdrawalIndex()
	// if err != nil {
	// 	return nil, errors.Wrap(err, "could not get next withdrawal index")
	// }
	// fmt.Println("이전 출금 인덱스 : ", nextIndex)
	if len(expectedWithdrawals) > 0 {
		// Update the next withdrawal index if this block contained withdrawals
		// state.next_withdrawal_index = WithdrawalIndex(latest_withdrawal.index + 1)
		...
		// 내가추가한 코드
		// fmt.Println("예상 출금 목록 갯수 : ", len(expectedWithdrawals))
		nextIndex2, err := st.NextWithdrawalIndex()
		if err != nil {
			return nil, errors.Wrap(err, "could not get next withdrawal index")
		}
		fmt.Println("최신 출금 인덱스", nextIndex2)
	}

	// fmt.Println("--- 6. 다음 출금 검증인 인덱스를 업데이트한다.")
	// // 내가추가한 코드
	// prevValidatorIndex, err := st.NextWithdrawalValidatorIndex()
	// if err != nil {
	// 	return nil, errors.Wrap(err, "could not get previous withdrawal validator index")
	// }
	// fmt.Println("이전 검증인 인덱스:", prevValidatorIndex)
	...
	if uint64(len(expectedWithdrawals)) < params.BeaconConfig().MaxWithdrawalsPerPayload {
		...
		// 내가추가한 코드
		// if len(expectedWithdrawals) > 0 {
		// 	fmt.Println("최근 검증인 인덱스:", expectedWithdrawals[len(expectedWithdrawals)-1].ValidatorIndex)
		// }
	...
	// 다음 검증인 인덱스 로그
	// fmt.Println("다음 검증인 인덱스:", nextValidatorIndex)
